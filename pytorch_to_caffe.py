...
def _split(raw, tensor, split_size, dim=0):
    ...
    if isinstance(split_size, int):
        ...
        slice_points = []
        prev = 0
        for i in split_size[:-1]:
            slice_points.append(prev + i)
            prev += i
        slice_param = caffe_net.pb.SliceParameter(
            axis=dim, slice_point=slice_points)
...
def _flatten(raw, tensor, start_dim=0, end_dim=-1):
    x = raw(tensor, start_dim, end_dim)
    layer_name = log.add_layer('flatten')
...
def _batch_norm(raw,
    ...
    if running_mean is None or running_var is None:
        ...
        layer1.batch_norm_param(use_global_stats=0, eps=eps, moving_average_fraction=1-momentum)
    else:
        layer1.batch_norm_param(use_global_stats=1, eps=eps, moving_average_fraction=1-momentum)
